...
// TODO RK #include "qelapsedtimer.h"
#include "qdatetime.h"
    ...
    for (int i = 0; pattern->tokens[i] != 0; ++i) {
        ...
        if (token == endifTokenC) {
            ...
            if (timeFormat == QLatin1String("process")) {
                    // TODO RK quint64 ms = pattern->timer.elapsed();
                    // message.append(QString::asprintf("%6d.%03d", uint(ms / 1000), uint(ms % 1000)));
            } else if (timeFormat ==  QLatin1String("boot")) {
                // just print the milliseconds since the elapsed timer reference
                // like the Linux kernel does
                /* TODO RK QElapsedTimer now;
                now.start();
                uint ms = now.msecsSinceReference();
                message.append(QString::asprintf("%6d.%03d", uint(ms / 1000), uint(ms % 1000)));
                */
            } else if (timeFormat.isEmpty()) {
                    // TODO RK message.append(QDateTime::currentDateTime().toString(Qt::ISODate));
            } else {
                // TODO RK message.append(QDateTime::currentDateTime().toString(timeFormat));
            }
#endif
