...
#ifdef QT_NO_QOBJECT
static inline QString tr(const char* str) { return str; }
#endif
/*!
...
bool QSaveFile::open(OpenMode mode)
{
    ...
    if (existingFile.exists() && !existingFile.isWritable()) {
        d->setError(QFileDevice::WriteError, tr("Existing file %1 is not writable").arg(d->fileName));
        d->writeError = QFileDevice::WriteError;
        return false;
    }

    if (existingFile.isDir()) {
        d->setError(QFileDevice::WriteError, tr("Filename refers to a directory"));
        d->writeError = QFileDevice::WriteError;
        return false;
    }
...
void QSaveFile::cancelWriting()
{
    ...
    if (!isOpen())
        return;
    d->setError(QFileDevice::WriteError, tr("Writing canceled by application"));
    d->writeError = QFileDevice::WriteError;
}
