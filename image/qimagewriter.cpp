...
#if !defined(QT_NO_IMAGEFORMATPLUGIN) && !defined(QT_NO_PLUGINS)
    ...
    const QByteArray &format)
{
    ...
    if (suffixPluginIndex != -1) {
        ...
        const int index = keyMap.key(QString::fromLatin1(suffix), -1);
        if (index != -1) {
            QImageIOPlugin *plugin = dynamic_cast<QImageIOPlugin *>(l->instance(index));
    ...
    if (!testFormat.isEmpty()) {
        ...
        for (int i = 0; i < keyCount; ++i) {
            QImageIOPlugin *plugin = dynamic_cast<QImageIOPlugin *>(l->instance(i));
...
{
#ifndef QT_NO_FILEENGINE
    QFile *file = dynamic_cast<QFile *>(d->device);
...
{
#ifndef QT_NO_FILEENGINE
    if (QFile *file = dynamic_cast<QFile *>(d->device)) {
                                  ...
                                  QList<QByteArray> *result)
{
    ...
    for (PluginKeyMapConstIterator it = keyMap.constBegin(); it != cend; ++it) {
        if (it.key() != i) {
            i = it.key();
            plugin = dynamic_cast<QImageIOPlugin *>(loader->instance(i));
        }
                                    ...
                                    QList<QByteArray> *result)
{
    ...
    for (int i = 0; i < pluginCount; ++i) {
        ...
        QImageIOPlugin *plugin = dynamic_cast<QImageIOPlugin *>(loader->instance(i));
