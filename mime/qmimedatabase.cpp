...
void QMimeDatabase::addMimeTypes(const QString& fileName)
{
    QMimeDatabasePrivate* d = QMimeDatabasePrivate::instance();
    QMutexLocker locker(&d->mutex);
    QMimeXMLProvider* xmlProvider = static_cast<QMimeXMLProvider *>(d->provider());
    xmlProvider->addFile(fileName);
}

static const char* ALL_FILES_FILTER  = QT_TRANSLATE_NOOP("Core", "All Files (*)");

QString QMimeDatabase::allFiltersString(QString* allFilesFilter)
{
    QMimeDatabase mdb;
    QSet<QString> uniqueFilters;
    foreach (const QMimeType &mt, mdb.allMimeTypes()) {
    ...
    filters.sort();
    static const QString allFiles = ALL_FILES_FILTER;
        // TODO QCoreApplication::translate("Core", ALL_FILES_FILTER);
    if (allFilesFilter)
...
QStringList QMimeDatabase::allGlobPatterns()
{
    QMimeDatabase mdb;
...
void QMimeDatabase::setGlobPatternsForMimeType(const QMimeType& mimeType, const QStringList& patterns)
{
    QMimeDatabasePrivate* d = QMimeDatabasePrivate::instance();
    QMutexLocker locker(&d->mutex);
...
void QMimeDatabase::magicRulesForMimeType(const QMimeType& mimeType, QMimeMagicRulesMap& out)
{
    QMimeDatabasePrivate* d = QMimeDatabasePrivate::instance();
    QMutexLocker locker(&d->mutex);
    return d->provider()->magicRulesForMimeType(mimeType, out);
}

void QMimeDatabase::setMagicRulesForMimeType(const QMimeType& mimeType, const QMimeMagicRulesMap& rules)
{
    QMimeDatabasePrivate* d = QMimeDatabasePrivate::instance();
    QMutexLocker locker(&d->mutex);
